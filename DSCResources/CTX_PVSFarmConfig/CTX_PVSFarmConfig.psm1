...
<#         $pvsStore = Get-PvsStore
    ...
    $targetResource = @{
        ...
        OtherStreamingIPs           = $pvsBootServer.Bootserver1_Ip.IPAddressToString, $pvsBootServer.Bootserver2_Ip.IPAddressToString, $pvsBootServer.Bootserver3_Ip.IPAddressToString,$pvsBootServer.Bootserver4_Ip.IPAddressToString           
        Ensure                      = "Absent"
    }
...
function Test-TargetResource {
    ...
    param (
        ...
        [Parameter()]
        [System.String[]] $OtherStreamingIPs = '',
        ...
        [Parameter(Mandatory)]
        [ValidateSet('Create', 'Join', 'AlreadyConfigured')]
        [System.String] $FarmConfiguration = 'Create',
...
function Set-TargetResource {
    ...
    if ($FarmConfiguration -like 'Create')
    {
        $PVSConfig += "FarmConfiguration=1" + "`r`n"   # 0 = farm already configures, 1= create farm, 2 = join farm
        ...
        $i = 2
        foreach ($ip in $OtherStreamingIPs) {
            $PVSConfig += "LS" + $i + "=" + $OtherStreamingIPs[$i - 2] + ",0.0.0.0,0.0.0.0,6910" + "`r`n"   
            ++$i
        }
    }

    elseif ($FarmConfiguration -like 'Join')
    {

    }

    elseif ($FarmConfiguration -like 'AlreadyConfigured')
    {
